...
function modelList() {
  return [
    ["openai", process.env.OPENAI_API_KEY, process.env.OPENAI_COACH_MODEL || "gpt-5.6-luna"],
    ["anthropic", process.env.ANTHROPIC_API_KEY, process.env.ANTHROPIC_COACH_MODEL || "claude-opus-5"],
    ["gemini", process.env.GEMINI_API_KEY, process.env.GEMINI_COACH_MODEL || "gemini-3.7-flash"],
    ["groq", process.env.GROQ_API_KEY, process.env.GROQ_COACH_MODEL || "qwen/qwen3.6-27b"]
  ].filter((x) => x[1]);
}
...
async function groq(key, model, system, user) {
  const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
    method: "POST",
    headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" },
    body: JSON.stringify({ model, temperature: 0.1, max_completion_tokens: 8000, messages: [{ role: "system", content: system }, { role: "user", content: user }] })
  });
  const payload = await response.json();
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const plans = planResults.map((r, i) => r.status === "fulfilled" ? { provider: planners[i][0], text: r.value.text } : null).filter(Boolean);
    if (!plans.length) return res.status(502).json({ error: "No se pudo generar un plan de implementación." });
    ...
    const coderProvider = planners.find((p) => p[0] === "openai") || planners.find((p) => p[0] === "gemini") || planners[0];
    const coded = await ask(coderProvider, CODER, coderUser);
    let patchSet;
