...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const table of TABLES) {
      ...
      tables[table] = probe.error ? {ok:false,error:probe.error.message,code:probe.error.code||null} : {ok:true};
    }
    const failed = Object.entries(tables).filter(([,v]) => !v.ok).map(([name]) => name);
    return res.status(200).json({ ok:failed.length===0, stage:"database", user:Boolean(auth.data?.user), tables, failed });
  } catch (error) {
