...
const MARKET_SOURCES: Record<string, { source: string; symbol: string }> = {
  sp500: { source: "av", symbol: "SPY" }, nasdaq: { source: "av", symbol: "QQQ" }, dow: { source: "av", symbol: "DIA" }, russell: { source: "av", symbol: "IWM" },
...
Deno.serve(async (request) => {
  ...
  if (request.method === "POST") {
    try { ids = (await request.json())?.ids || []; } catch (_) { ids = []; }
  } else {
  ...
  const results = await Promise.all(requested.map(async (id) => {
    ...
    try {
      const quote = market.source === "cg" ? await coinGecko(market.symbol) : await alphaVantage(market.symbol);
  ...
  return json({ userId: user.id, fetchedAt: new Date().toISOString(), data: results });
});
