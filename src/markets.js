...
// GitHub Pages es una web estática y el navegador puede bloquear la petición
// directa a Yahoo por CORS. Probamos primero directo y después proxies CORS.
async function fetchJson(url){
  ...
  throw lastError||new Error("Fuente de mercado no disponible");
}

async function yahoo(symbol){
  const url=YAHOO+encodeURIComponent(symbol)+"?range=1d&interval=5m&includePrePost=false";
  const d=await fetchJson(url),m=d?.chart?.result?.[0]?.meta;
  ...
  if(!Number.isFinite(price))throw new Error("Cotización no disponible");
  return {price,change:prev?((price-prev)/prev)*100:NaN,currency:m.currency||""};
}
async function crypto(id){
  const url=`${COINGECKO}/simple/price?ids=${id}&vs_currencies=eur&include_24hr_change=true`;
  const x=(await fetchJson(url))[id];
