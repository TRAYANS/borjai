...
const SYNCING_KEY = "borjai:v14:syncing";
const MAX_DAYS = 1825;
let syncing = false;
let timer = null;
...
function writeLocal(state) {
  ...
  syncing = true;
  try {
    originalSetItem.call(localStorage, LOCAL_STORAGE_KEY, JSON.stringify(state));
  } finally {
    syncing = false;
  }
}
...
function withDailySnapshot(state) {
  ...
  const value = calculateWealth(state);
  ...
  if (found) {
    ...
    found.month = month;
    found.value = value;
    found.source = "daily-sync";
  } else {
    snapshots.push({ date, month, value, source: "daily-sync" });
  }
  snapshots.sort((a, b) => String(a.date || a.month).localeCompare(String(b.date || b.month)));
  state.snapshots = snapshots.filter((s) => {
    const d = String(s.date || (s.month ? `${s.month}-01` : "")).slice(0, 10);
...
async function syncState(state) {
  ...
  try {
    ...
    if (!client) return;
    const repo = createSupabaseRepository(client, () => state);
...
function scheduleSync(state) {
  clearTimeout(timer);
  timer = setTimeout(() => syncState(state), 300);
}
...
async function reconcileOnBoot() {
  ...
  try {
    ...
    if (local && localIsNewer(local, remote)) {
      await repo.saveState(withDailySnapshot(local));
    } else if (!local || !localIsNewer(local, remote)) {
      const prepared = withDailySnapshot(remote);
      writeLocal(prepared);
    }
  } catch (_) {
    // The main financial API remains responsible for its own local fallback.
  }
}
  ...
  localStorage.setItem = function(key, value) {
    ...
    if (!syncing && key === LOCAL_STORAGE_KEY) {
      try {
        ...
        if (state?.version === 1) scheduleSync(state);
      } catch (_) {
        // Ignore malformed storage writes; the main app handles validation.
      }
    }
  };
}
