...
// Fuente única de verdad para la salud financiera. Se conserva la misma lógica
// que utiliza la pantalla Inicio para que Inicio, Borjai 2.0 y el monitor den
...
export function healthScore(data={},month=monthKey()){
  ...
  const assets=data.assets||[],assetGroup=(a)=>normalizeName(a.group||a.category||a.type),assetValue=(a)=>num(a.current_value??a.value),groupValue=(names)=>assets.filter(a=>names.some(n=>assetGroup(a).includes(n))).reduce((s,a)=>s+assetValue(a),0),invested=groupValue(["inversion"])+groupValue(["cripto","crypto"])+groupValue(["oro","metal"]),investedRatio=b.netWorth>0?invested/b.netWorth:0;
  const allocationValues=[];for(const a of data.accounts||[])allocationValues.push(num(a.current_balance??a.balance));for(const a of assets)allocationValues.push(assetValue(a));const maxAllocation=b.netWorth>0?allocationValues.reduce((m,v)=>Math.max(m,Math.max(0,v)/b.netWorth),0):0;
  ...
  const cryptoRatio=b.netWorth>0?groupValue(["cripto","crypto"])/b.netWorth:0;
  ...
  if(!hasEconomic)return{score:0,savingsRate:0,monthsLiquidity:0,liquidityMonths:0,debtRatio:0,spendingRatio:0,investedRatio:0,target:0,reserveTarget:0,confidence:"baja",label:"Sin datos",metrics:{income:0,expense:0,invested:0,savings:0,rate:0},parts};
  ...
  return{score:clamp(score,0,100),savingsRate:totals.income>0?totals.net/totals.income:0,monthsLiquidity:liquidityMonths,liquidityMonths,target:reserveTarget,reserveTarget,debtRatio,spendingRatio,investedRatio,confidence:populated>=4?"alta":populated>=2?"media":"baja",label:"Calculada con datos reales",metrics:{income:totals.income,expense:totals.expenses,invested,savings:totals.net,rate:totals.income?totals.net/totals.income:0},parts};
}
