...
function screen(mode, message = "") {
  ...
  const copy = activation ? "Esta instalación se vinculará a tu única cuenta privada. No se pueden crear otras cuentas." : recover ? "Introduce una nueva contraseña para recuperar el acceso." : forgot ? "Te enviaremos un enlace seguro a tu correo para crear una nueva contraseña." : "Accede a tus datos financieros desde cualquier navegador o dispositivo.";
  document.body.innerHTML = `
    <main class="borjai-auth-shell"><section class="borjai-auth-card">
      ...
      <form id="borjai-auth-form" class="borjai-auth-form">
        ${(activation || mode === "login" || forgot) ? `<label>Email<input id="auth-email" type="email" autocomplete="username" required value="${esc(ALLOWED_EMAIL)}" ${activation ? "readonly" : ""}></label>` : ""}
  ...
  document.getElementById("borjai-auth-form").addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (activation) {
        ...
        await client.auth.updateUser({ email, password }, { emailRedirectTo: `${PUBLIC_APP_URL}${location.pathname}` });
        screen("login", "Te hemos enviado un correo de confirmación. Confirma tu correo y después inicia sesión con tu contraseña."); return;
      }
