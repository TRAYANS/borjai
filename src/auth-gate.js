...
function screen(mode, message = "") {
  ...
  document.body.innerHTML = `
    <main class="borjai-auth-shell">
      <section class="borjai-auth-card">
        <div class="borjai-auth-brand"><span>B</span>Borja<span>AI</span></div>
        <div class="borjai-auth-kicker">CUENTA PRIVADA</div>
        <h1>${activation ? "Activa tu cuenta" : mode === "recover" ? "Nueva contraseña" : "Iniciar sesión"}</h1>
        <p class="borjai-auth-copy">${activation ? "Vincula esta instalación con tu correo y conserva tus datos en una única cuenta." : mode === "recover" ? "Introduce una nueva contraseña para recuperar el acceso." : "Accede a tus datos financieros desde cualquier navegador o dispositivo."}</p>
        ${message ? `<div class="borjai-auth-message">${esc(message)}</div>` : ""}
        <form id="borjai-auth-form" class="borjai-auth-form">
          ${!mode || mode === "login" ? `<label>Email<input id="auth-email" type="email" autocomplete="username" required placeholder="tu@email.com"></label>` : ""}
          ${activation ? `<label>Email<input id="auth-email" type="email" autocomplete="username" required placeholder="tu@email.com"></label>` : ""}
          ${activation || mode === "login" || mode === "recover" ? `<label>Contraseña<input id="auth-password" type="password" autocomplete="${mode === "login" ? "current-password" : "new-password"}" minlength="10" required placeholder="Mínimo 10 caracteres"></label>` : ""}
          ${activation ? `<label>Confirma contraseña<input id="auth-confirm" type="password" autocomplete="new-password" minlength="10" required placeholder="Repite la contraseña"></label>` : ""}
          <button class="borjai-auth-primary" type="submit">${activation ? "Activar mi cuenta" : mode === "recover" ? "Guardar nueva contraseña" : "Entrar"}</button>
        </form>
  ...
  document.getElementById("borjai-auth-form").addEventListener("submit", async (event) => {
    ...
    try {
      if (mode === "forgot") {
        ...
        screen("login", "Si existe una cuenta con ese correo, recibirás un enlace para restablecer la contraseña.");
        return;
      }
      if (mode === "recover") {
        if (password !== confirm && document.getElementById("auth-confirm")) throw new Error("Las contraseñas no coinciden.");
      ...
      if (mode === "activate") {
        ...
        if (!user?.is_anonymous) throw new Error("Esta sesión ya pertenece a una cuenta permanente. Usa Iniciar sesión.");
        await client.auth.updateUser({ email });
        sessionStorage.setItem(PENDING_KEY, JSON.stringify({ email }));
        screen("login", "Te hemos enviado un correo de confirmación. Confirma el correo y vuelve a BORJAI para terminar de activar tu cuenta y establecer la contraseña.");
        return;
      }
...
async function start() {
  ...
  if (!hasSupabaseConfig(config)) {
    screen("login", "La autenticación de Supabase no está configurada todavía.");
    return;
  }
  ...
  if (pending && user?.email_confirmed_at) {
    screen("recover", `Correo confirmado: ${esc(user.email || pending.email)}. Ahora establece tu contraseña.`);
    return;
  }
