...
const ALLOWED_EMAIL = "borjabuzquin@gmail.com";
...
function isAllowedEmail(value) { return normalizeEmail(value) === ALLOWED_EMAIL; }

function screen(mode = "login", message = "") {
  const forgot = mode === "forgot";
  const recover = mode === "recover";
  const title = recover ? "Nueva contraseña" : forgot ? "Recuperar contraseña" : "Iniciar sesión";
  const copy = recover ? "Introduce una nueva contraseña para recuperar el acceso." : forgot ? "Te enviaremos un enlace seguro a tu correo para crear una nueva contraseña." : "Accede a tus datos financieros desde cualquier navegador o dispositivo.";
  document.body.innerHTML = `
    <main class="borjai-auth-shell"><section class="borjai-auth-card">
      ...
      <form id="borjai-auth-form" class="borjai-auth-form">
        ${(mode === "login" || forgot) ? `<label>Email<input id="auth-email" type="email" autocomplete="username" required value="${esc(ALLOWED_EMAIL)}"></label>` : ""}
        ${(mode === "login" || recover) ? `<label>Contraseña<input id="auth-password" type="password" autocomplete="${mode === "login" ? "current-password" : "new-password"}" minlength="10" required placeholder="Mínimo 10 caracteres"></label>` : ""}
        ${recover ? `<label>Confirma contraseña<input id="auth-confirm" type="password" autocomplete="new-password" minlength="10" required placeholder="Repite la contraseña"></label>` : ""}
        <button class="borjai-auth-primary" type="submit">${recover ? "Guardar nueva contraseña" : forgot ? "Enviar enlace" : "Entrar"}</button>
      </form>
      <div class="borjai-auth-links">
        ${!recover && !forgot ? `<button type="button" data-mode="forgot">He olvidado mi contraseña</button>` : ""}
  ...
  document.getElementById("borjai-auth-form").addEventListener("submit", async (event) => {
    ...
    const email = normalizeEmail(document.getElementById("auth-email")?.value); const password = document.getElementById("auth-password")?.value || ""; const confirm = document.getElementById("auth-confirm")?.value || "";
    try {
      if (!isAllowedEmail(email)) throw new Error("Este correo no está autorizado para BORJAI.");
      if (forgot) { await client.auth.resetPasswordForEmail(email, { redirectTo: `${PUBLIC_APP_URL}${location.pathname}` }); screen("login", "Si la cuenta existe, recibirás un enlace seguro para restablecer la contraseña."); return; }
