...
let selected = localStorage.getItem("borjai:wealth-range") || "1y";
let snapshots = [];
let client = null;
let loading = false;
...
function normalizeRows(rows) {
  return (rows || [])
    .map(row => ({
      date: row.snapshot_date || row.month || row.date,
...
function aggregate(rows, period) {
  ...
  const bucket = period.id === "max" || period.days >= 1095
    ? "month"
    : period.days >= 180
      ? "week"
      : "day";
...
function buildChart(rows) {
  ...
  return `
    <svg class="wealth-range-svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" role="img" aria-label="Evolución del patrimonio">
      ...
      ${labels.map(p => `<text x="${p.x}" y="${height - 9}" text-anchor="${p.x < width * .2 ? "start" : p.x > width * .8 ? "end" : "middle"}" class="wealth-range-label">${escapeHtml(dateLabel(p.date))}</text>`).join("")}
    </svg>
  `;
}
...
function ensureStyles() {
  ...
  style.textContent = `
    ...
    .wealth-range-tabs{display:flex;gap:4px;padding:4px;border:1px solid var(--line,#292c33);border-radius:10px;background:var(--bg-raised,#0d0f13);overflow:auto}
    ...
    .wealth-range-empty{display:grid;min-height:205px;place-items:center;text-align:center;color:var(--muted,#9da3ad);font-size:13px;border:1px dashed var(--line,#292c33);border-radius:10px}
...
function renderIntoPanel(panel) {
  ...
  const old = panel.querySelector(".wealth-range-enhanced");
  if (old) old.remove();
  ...
  enhanced.innerHTML = `
    ...
    <div class="wealth-range-chart">
      ${rows.length >= 2 ? buildChart(rows) : `<div class="wealth-range-empty">No hay histórico suficiente para este periodo.<br><small>Cuando existan más snapshots de patrimonio, este tramo se rellenará automáticamente.</small></div>`}
    </div>
  `;

  enhanced.querySelectorAll("[data-wealth-period]").forEach(button => {
    button.addEventListener("click", () => {
      selected = button.dataset.wealthPeriod;
      localStorage.setItem("borjai:wealth-range", selected);
...
async function init() {
  ...
  const observer = new MutationObserver(() => {
    const panel = document.querySelector(".chart-panel");
    if (panel && !panel.querySelector(".wealth-range-enhanced")) refresh();
  });
